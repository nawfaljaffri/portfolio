...
with open('ptr_target.txt', 'r') as f:
    target_ptr = f.read()

with open('dir_target.txt', 'r') as f:
    target_dir = f.read()

# THEMES chunk replacement
old_themes = """  { name: 'WHITE',  fg: '#FFFFFF', bg: '#000000', dim: '#666666', bloom: 0.46, radius: 1.80, thresh: 0.30, burnIn: 0.80, bright: 1.115, satur: 1.24, crush: 0.30, grain: 0.15, curve: 0.20 },"""
new_themes = """  { name: 'WHITE',  fg: '#000000', bg: '#FFFFFF', dim: '#888888', bloom: 0.20, radius: 0.50, thresh: 0.50, burnIn: 0.10, bright: 1.00, satur: 1.00, crush: 0.10, grain: 0.05, curve: 0.05 },"""
if old_themes in content:
    content = content.replace(old_themes, new_themes)
else:
    print("Warning: old_themes not found")

# DRAW chunk replacement
        ...
        if (node.type === 'page') {
            if (node.content) {
                node.content.forEach((line, i) => {
                    const y = boxY + 1 + i - uiState.scrollOffset;
            ...
            if (!uiState.viewingProjectDetail) {
                if (node.children) {
                    node.children.forEach((proj, i) => {
                        const lineDist = 2;
                        const y = boxY + 1 + i * lineDist - uiState.scrollOffset;
                        if (y >= boxY + 1 && y <= boxY + 15) {
                ...
                if (proj.desc) writeUI(pStartX, boxY + 13, `${proj.desc.replace(/\\\\n/g, ' ').substring(0, pWidth)}`, 0);
                writeUI(pStartX, boxY + 15, `Stack: ${proj.lang}    |    Links: ${proj.links || '[GitHub]'}`, 0);
            }
        }
    }
"""
if target_draw in content:
    content = content.replace(target_draw, new_draw)
else:
    print("Warning: target_draw not found")

# PTR chunk replacement
        ...
        if (gridY >= boxY && gridY < boxY + 17) {
            if (gridX > 0 && gridX < navW) {
                ...
                if (node && node.type === 'folder' && !uiState.viewingProjectDetail) {
                    const lineDist = 2;
                    const idx = Math.floor((gridY - (boxY + 1) + uiState.scrollOffset) / lineDist);
                    if (idx >= 0 && idx < node.children.length && (gridY - (boxY + 1) + uiState.scrollOffset) % lineDist === 0) {
                        playTick();
...
"""
if target_ptr in content:
    content = content.replace(target_ptr, new_ptr)
else:
    print("Warning: target_ptr not found")

# DIR chunk replacement
...
if target_dir in content:
    content = content.replace(target_dir, new_dir)
else:
    print("Warning: target_dir not found")
